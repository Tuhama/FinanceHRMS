...
<table  id="healthleave_table"  class="display" cellspacing="0" width="400" >
    <thead>
        ...
        <c:forEach var="healthleave_type" items="${healthleave_types}" varStatus="iter">
            ...
            <tr>
                <td>${healthleave_type.id}</td>
                <td>${healthleave_type.name}</td>
                <td><input type="button" value="تعديل" name="edit_b" onclick="show_edit_dialog('${healthleave_type.id}', '${healthleave_type.name}')"/></td>        
                <td><input type="button" value="حذف" name="delete_b" onclick="show_delete_dialog('${healthleave_type.id}')"/></td>             
            </tr>
        </c:forEach>   
</table>   
</c:if>
...
<div id="edit_dialog" title="تعديل" >
    <form action="editHealthleavetype" method="post" id="e_d_form">
        <table>
...
<div id="delete_dialog" title="حذف">
    <form action="deleteHealthleavetype" method="post" id="d_d_form">
...
<div id="add_dialog" title="إضافة">
    <form id="form1" name="form1" method="post" action="<c:url value='addHealthleavetype'/>" accept-charset="UTF-8">
