...
<c:if test="${!empty unpaid_vacation_types}">
    <table   id="unpaidv_table"  class="display" cellspacing="0" width="400" >
        ...
        <c:forEach var="unpaidvacationtype" items="${unpaid_vacation_types}" varStatus="iter">

            <tr>
                <td>${unpaidvacationtype.id}</td>
                <td>${unpaidvacationtype.name}</td>
                <td><input type="button" value="تعديل" name="edit_b" onclick="show_edit_dialog('${unpaidvacationtype.id}', '${unpaidvacationtype.name}')"/></td>        
                <td><input type="button" value="حذف" name="delete_b" onclick="show_delete_dialog('${unpaidvacationtype.id}')"/></td>             
            </tr>
...
<div id="edit_dialog" title="تعديل" >
    <form action="editUnpaidvacationtype" method="post" id="e_d_form">
        <table>
...
<div id="delete_dialog" title="حذف">
    <form action="deleteUnpaidvacationtype" method="post" id="d_d_form">
...
<div id="add_dialog" title="إضافة">
    <form id="form1" name="form1" method="post" action="<c:url value='addUnpaidvacationtype'/>" accept-charset="UTF-8">
